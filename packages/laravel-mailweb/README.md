...
Files are equally direct:

```php
$hero = MailWeb::enclosure(resource_path('mailweb/hero.png'), 'hero');
$manifesto = MailWeb::enclosure(resource_path('mailweb/manifesto.txt'), 'manifesto');

return MailWeb::page('Parcel')
    ->image($hero, 'Correspondence piled alarmingly high', 'hero')
    ->attachment($manifesto, 'Download the manifesto', 'Plain text');
```

Filename, actual media type, byte size, SHA-256 digest, base64 JSON representation, and SMTP MIME parts are generated by the package. If the client advertises the digest as present, bytes are omitted while the exact resource descriptor remains.
